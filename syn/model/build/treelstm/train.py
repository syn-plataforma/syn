...
from syn.helpers.task import get_trees_column_name, get_attention_vectors_column_name, get_structured_data_column_name
from syn.helpers.treelstm.dataloader import DataLoader
from syn.model.build.treelstm.dynettask import Train
...
if __name__ == "__main__":
    ...
    train = DataLoader(
        ...
        trees_columns=get_trees_column_name(
            input_params['dataset']['task'], input_params['dataset']['corpus']
        ),
        attention_vectors_columns=get_attention_vectors_column_name(
            input_params['dataset']['task'], input_params['dataset']['corpus']
        ),
    ...
    dev = DataLoader(
        ...
        task=input_params['dataset']['task'],
        corpus=input_params['dataset']['corpus'],
        dataset_name='balanced_dev' if input_params['dataset']['balance_data'] else 'dev',
        trees_columns=get_trees_column_name(
            input_params['dataset']['task'], input_params['dataset']['corpus']
        ),
        attention_vectors_columns=get_attention_vectors_column_name(
    ...
    trainer = Train(w2i, word_embed, train, dev, input_params)
