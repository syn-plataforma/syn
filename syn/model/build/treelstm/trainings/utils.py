...
def parse_TAG_one_hot(sexpr, reverse_vocab=None, can_fail=False):
...
def parse_ptb_sexpr_one_TAG(sexpr, reverse_vocab=None, can_fail=False):
    result, _ = _parse_ptb_sexpr_one_TAG(sexpr.strip(), 0, reverse_vocab, can_fail)
    if can_fail and result is None:
        return None
    if len(result) == 1:
        return result[0]
...
def _parse_ptb_sexpr_one_TAG(sexpr, position, reverse_vocab):
    if sexpr[position] != "(":
        raise ValueError("Missing opening parenthesis")
    i = position + 1
    nonterminal_done = False
    ...
    while i < len(sexpr):
        if sexpr[i] == " " or sexpr[i] == "\n":
            ...
            child, child_end = _parse_ptb_sexpr_one_TAG(sexpr, i, reverse_vocab)
            if len(child) == 1:
                node.append(child[0])
            else:
                node.append(child)
...
def parse_ptb_sexpr_TAG(sexpr, reverse_vocab=None, can_fail=False):
    i = 0
    inicio = ""
    inicio_tag = False
    list_list_tag = list()
        ...
        if inicio == '(ROOT (':
            ...
            while sexpr[i] != ")":
                if sexpr[i] == "(":
                    ...
                    string = string + sexpr[i]
                    i = i + 1
            list_list_tag.append(string)
        else:
            inicio += inicio + sexpr[i]
            i = i + 1


def _parse_ptb_sexpr_TAG(sexpr, position, reverse_vocab, can_fail):
    print(sexpr)
    if sexpr[position] != "(":
        raise ValueError("Missing opening parenthesis")
    i = position + 1
    ...
    while i < len(sexpr):
        if sexpr[i] == " " or sexpr[i] == "\n":
            ...
            child, child_end = _parse_ptb_sexpr_TAG(sexpr, i, reverse_vocab, can_fail)
            if can_fail and child is None:
                return None, None
            if len(child) == 1:
                node.append(child[0])
