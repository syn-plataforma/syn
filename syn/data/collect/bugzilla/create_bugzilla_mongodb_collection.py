...
from syn.helpers.bugzilla.BugzillaHelper import get_bugzilla_bugs_by_date_range, get_input_params
...
def main():
    ...
    for month in range(input_params['bz_api_params'].start_month, input_params['bz_api_params'].end_month + 1):
        max_month = month + 1
        if max_month > 12:
            max_month = 1
            max_year += 1
        bugs = get_bugzilla_bugs_by_date_range(
            ...
            min_creation_ts=f"{input_params['bz_api_params'].year}-{str(month).zfill(2)}-01",
